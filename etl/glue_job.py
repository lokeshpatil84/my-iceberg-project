...
s3_bucket = spark.conf.get("spark.sql.catalog.glue_catalog.warehouse", "s3://default-bucket/warehouse/")
...
def transform_data(df: DataFrame) -> DataFrame:
    ...
    transformed_df = df.withColumn("salary_category", 
...
def write_to_iceberg(df: DataFrame, table_name: str, database_name: str):
...
def perform_analytics(df: DataFrame):
